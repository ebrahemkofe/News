...
public class CustomPagerAdapterSports extends PagerAdapter {
    ...
    public Object instantiateItem(ViewGroup container, final int position) {
        ...
        String link=list.get(position).getNews().get(1).getCategory_posts().get(position).getPost_img();
        Glide.with(mContext).load(link).into(image);
        titl.setText(list.get(position).getNews().get(1).getCategory_posts().get(position).getPost_title());
        disc.setText(list.get(position).getNews().get(1).getCategory_posts().get(position).getDescription());
